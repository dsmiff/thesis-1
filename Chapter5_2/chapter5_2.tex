...
simulation in \GEANTFOUR, and detector hits `digitized' in order to simulate the
response of detector electronics. Prior to digitization, a number of
`MinimumBias' events are
overlayed in order to reproduce LHC run conditions. Event-level weights
...
In order to increase statistics this analysis makes use of MC samples binned
according to
...
inclusive sample cross-sections. However, recent CMS studies
\cite{RobXS} have shown that the
...
where unphysical steps are present in the ratio between the \zj binned sample 
...
a comparison to be made, namely \wj and \dyj. For these two cases, the 
derivation of corrections for each \HTpart binned sample is trivial. However,
for \zj no such inclusive sample exists. This sample is 
compared with the inclusive \dyj sample, where the overall normalisation is 
set as the relative cross section of \zinv to
$Z \to \ell\ell$, 0.505. An example ratio plot is
shown in figure~\ref{fig:xsec_study_after}, where a constant ratio between the 
samples is found following the application of corrections to the individual
...
MC processes must be derived. Corrections are taken as the data to MC ratio
in the
$150 \leq \HT < 200$~\gev sideband region, in a given control sample with a 
targeted selection, designed to yield a pure sample of a background process.
A summary of the selections, their relevant purities and the derived
corrections are given in table~\ref{tab:ht_sideband}.
...
following their application (discussed in section~\ref{sec:closure_tests}).
...
This method and it's validation are described in detail in
