...
objects used in this analysis are provided by the Physics Object Groups (\emph{POG's})
...
\begin{table}[!ht]
  \caption{Criteria for the ``loose'' jet identification working point.\label{tab:jet_id_loose}}
  \footnotesize
  \begin{center}
    \begin{tabular}{ll}
      \hline
...
the tracker using the muon \emph {POG's} Tight working point
definition~\cite{ref:muon-id}, as summarised in Table~\ref{tab:muon-id}. This definition is
...
This Tight working point ID is defined by the \emph{POG}
group~\cite{ref:photonidtwiki} and used for both the photon
...
The Electron \emph{POG's} Loose working point ID~\cite{ref:electronidtwiki} is
...
\clearpage
\noindent
requirements are specifically for the rejection of electrons originating from photons converted
into $e^-e^+$ pairs.
...
hadronic energy, total missing transverse hadronic energy vector and total missing transverse
hadronic energy (`MHT').
...
and leptonically decaying W bosons where the corresponding lepton has failed the
identification criteria for whatever reason.
