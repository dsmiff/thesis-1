...
this, events are required to have a low ratio of the two variables, specifically
\mhtmet < 1.25.
...
lost as a result. To protect against this the angular seperation between the 
event \mht and a given jet is used, defined as:
...
A small value of $\Delta \phi^*_j$ indicates that the vector of the $j_{th}$ jet
is aligned with the \mht vector, implying the jet is mis-measured. Events are 
vetoed if a jet with $\Delta \phi^*_j<0.5$ is within $\Delta R < 0.3$ of a known
...
Example distributions are shown in figure~\ref{fig:datamc_had_inc}, comparing data 
against MC.
