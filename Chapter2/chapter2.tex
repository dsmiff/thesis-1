...
new particles (sparticles). The simplest implementation of this symmetry is the
Minimally
Supersymmetryic Standard Model (MSSM) - a so-called `complete' model, containing
~100 free-parameters respresenting masses, phases and mixing angles (REF).
...
Given the lack of experimental evidence of super-particles, sparticles must
have higher masses than their SM partners, implying that Supersymmetry is not
an exact symmetry and must be broken by some mechanism - of which many exist
REF REF.
...
\subsubsection{Dark Matter}
In some supersymmetric theories, a new conserved quantum number $R$
...
$R_P$ is a multiplicative quantum number, therefore as a consequence sparticles
...
components of a given theory (e.g. mass spectra and mixing angles), the large
number of free parameters becomes un-manageable for experimental interpretations
of data. Even in `compressed' models, such as the Compressed-MSSM (CMSSM) with 5
...
experimentalists, while remaining universally of use to theorists, the
Simplified Model Spectra (SMS) was developed. SMS models consists of simplified
decay scenarios which can be later considered within any complete model.
Typically SMS models consist of either squark or gluino pair-production, with a
subsequent decay to a given final state with a 100\% branching fraction. This
moves experimental searches away from `model-specific' interpretations, and
towards more `signature-specific' interpretations.

Such models generally contain only two free parameters, normally the mass of a
