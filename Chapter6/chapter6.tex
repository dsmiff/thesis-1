...
[mention non-dependency on HT?]

\begin{table}[!h]
  \caption{Muon trigger efficiencies (\%) listed by \HT bin and \nj category.}
...
\alphat, subsequently leaving a very pure sample of QCD MJ events (SHOW PLOTS?).
...
where fits are made for $n=0$ and also where $n$ is allowed to float as a free 
parameter within the range $0-2$ in order to span the range from flat to a 
gaussian form. Example distributions and fits are shown in FIGURE.

Subsequent predictions are made and compared with the relevant EWK background 
