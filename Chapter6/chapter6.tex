...
The \mj control sample is constructed by selecting a single muon and associated 
...
using a tag-and-probe method, in bins of the muon \Pt and $\eta$, as summarised 
...
due to trigger requirements. The transverse mass of the W, reconstructed by the 
$\mu$ and the \met (originating from the $\nu_{\mu}$), is required to be in a 
loose window around $m_Z$, $30 < M(\mu, \met) < 125\gev$, in order to surpress 
...
present can cause the positive trigger decision, summarised in 
...
similar kinematics (once the $\gamma$ is ignored from the event) and larger 
production cross section compared to \mmj. Given the trigger thresholds, the \gj
sample cannot make predictions for $\HT<375\gev$, and so is complimentarily used
alongside the \mmj sample.
...
%       the \verb!HLTPhoton150! trigger that records events that
%       satisfy the \gj selection criteria, $E_{T}^{photon} >
%       165\gev$, $\HT > 375\gev$, and \njlow (Left) and \njhigh
%       (Right). REWORD}
