...
process. For each region, Transfer Factors (TF) are 
constructed from yields in MC in order to extract a prediction in the signal 
...
Predictions made using this technique are considered as ``primitive'' 
...
\end{equation}
% 
The control regions are statistically independent and each used for predicting 
specific background processes.

MC yields, following the application of all MC correction factors described in
Chapter~\ref{ch:samples}, are defined by process from the following samples:
W + jets (\numw),
\ttbar + jets (\numtt), DY + jets (\numdy), $\gamma$ + jets(\numgam),
...
regions used to determine a background prediction as a function of each analysis
category. For $\nb \leq 1$, the \mj control region is used to predict the
...
with the \mmj and \gj control regions, using only the \zinv MC yields:
% 
\begin{equation}
N_{MC}^{signal}\big(\HT, \nj, \nb \leq 1 \big) = \numzinv .
...
\end{equation}
%
For $\nb \geq 2$, the \mj region is used to produce a prediction for all 
...
The \mmj and \gj control regions are not used beyond the $\nb \leq 1$ categories
due to the statistical limitations of such regions at high b tag multiplicities.

It should be noted that although two separate control regions are 
...
\begin{table}[ht!]
  \caption{The control regions used to produce SM background predictions for each 
  analysis category (\nb, \nj).}
...
mismodelling, which could affect analysis acceptance, and mismodelling of 
...
%********************************** % Second Section  *************************************
\section{Control regions}  %Section - 1.2
\label{sec:background_control}

Control region definitions are designed such that they are as kinematically
similar as possible
...
and minor kinematic cuts, aimed at enriching the control regions in certain processes.
% The samples themselves are statistically independent, and orthogonal to the 
% signal region
Due to the selection of the tagged lepton or photon, the control regions are
orthogonal to the signal region and therefore minimise any
possible signal contamination. However, a full treatment of the
signal-contamination and region cross-correlation is taken into account in the
...
and their trigger details.

\subsection{\mj}
\label{sec:mujets_control_sample}

The \mj control region is constructed by selecting a single muon with associated 
...
loose window around the W mass, $30 < M(\mu, \met) < 125$~\gev. Events are vetoed if
...
Specifically for the \mj (and \mmj) control regions, no \alphat requirement is
made in order to increase the statistics and therefore the predictive power of 
the regions. This is possible as other requirements, in particular the 
requirement of
...
\begin{figure}[h!]
  \centering
    ...
    Comparison of data with MC for the \mj control selection. The plots show 
    an inclusive selection, $\HT>200$~\gev, $\nj\geq2, \nb\geq0$.
    }
\end{figure}

\subsection{\mmj}
The \mmj region is constructed to predict background contributions from \zinv 
...
The region is used to provide low \HT coverage (\HT > 200~\gev) for the \zinv background 
prediction, where the \gj region (Section~\ref{sec:gjets_control_sample})
is unable to do so.

\subsubsection{Triggers}
The trigger used is the same as for the \mj region, as described in
...
\begin{figure}[t]
  \centering
    ...
    Comparison of data with MC for the \mmj control selection. The plots 
    show an inclusive selection, $\HT>200$~\gev, $\nj\geq2, \nb\geq0$.}
\end{figure}

The selection for the \mmj region is very similar to that of the \mj region, 
described in Section~\ref{sec:mujets_control_selection}, with differences chosen
to enrich the region in Z bosons decaying to pairs of muons in the kinematic 
...
$i$ and every jet $j$. Similarly as in the \mj region selection, no \alphat
requirement is made.
...
The \gj region is used to predict the \zinv background contribution, given its 
...
region cannot make predictions for $\HT<375$~\gev, and so is complimentary to
the \mmj region prediction.
...
\begin{figure}[!h]
  \centering
    ...
    Comparison of data with MC for the \gj control selection. The plot shows 
    an inclusive selection, $\HT>200$~\gev, $\nj\geq2, \nb\geq0$.}
\end{figure}
...
(CTs) have been designed. The CT method works by constructing a TF to
extrapolate from one sub-region of a particular control region into another 
control region sub-region. This prediction method is identical to that described
...
The first test, represented by open circles, inspects the modelling of the \alphat 
variable in the \mj control region. In the analysis a prediction is made 
between the \mj region, which has no \alphat requirement, and the 
signal region, with its tight \alphat requirement. This particular test probes
...
shown here, is also performed for the \mmj control region.

The next two tests, represented by crosses and open squares, probe the
different b tag multiplicities in the \mj control region. The b tag 
...
is made from both the \gj and \mmj regions, and so a test is constructed to 
...
closure. The only exception is the 0 b-jets \ra 1 b-jet
(\mj) test for the
\njhigh category which has a sub-optimal goodness of fit value for the zeroeth order test. This is 
attributed to a downwards fluctuation in the adjacent 575-675~\gev bin. 
Also shown in Table~\ref{tab:syst-fits-ge4j} are significantly improved
fit parameters observed when the same fit is made while ignoring this bin.
This leads to the conclusion that this originates from
a statistical fluctuation as opposed to a systematic bias. Despite this
additional procedure however, it is worth noting this is the only test out of
the large ensemble considered indicating a poor fit, further
adding to the conclusion that this is indeed due to a statistical fluctuation.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ llrccccrc }
    ...
    0 b-jets \ra 1 b-jet (\mj)                & Times    & $-0.06 \pm 0.02$                 & 24.8     & 10     & 0.01      &  & $-4.1 \pm 1.2$    & 0.02      \\ 
    0 b-jets \ra 1 b-jet (\mj)$^{ \dag}$      & Times    & $-0.04 \pm 0.02$                 & 14.9     & 9      & 0.10      &  & $-3.9 \pm 1.3$    & 0.78      \\ 
...
\begin{table}[!ht]
  ...
  modelling in MC, for each control region.} 
  ...
  \begin{tabular}{ llrccccrc }
    ...
    \hline
           &                   & \multicolumn{4}{c}{Constant fit} &          & \multicolumn{2}{c}{Linear fit}                        \\
    \cline{3-6}\cline{8-9}
    Region & Symbol            & Best fit value                   & $\chi^2$ & d.o.f. & $p$-value &  & Slope ($10^{-4}$) & $p$-value \\
    \hline                                                                                                            
...
Uncertainties are calculated by summing in quadrature the weighted mean and sample 
...
hadronic energy vector calculated using all other remaining jets in the event. An advantage
of this variable with respect to the often used
$\Delta \phi(\overrightarrow{\Pt}_j, \mhtvect)$, is its detection of spurious missing
energy vectors caused
by both under-measurements or over-measurements of a jets energy. In the case
...
the same direction, giving a low \dphistar.
...
\ref{fig:hotspots_1d_withdeadECAL}, the hotspot areas and the corresponding
outliers are removed. The lack of localised high-count regions or
...
\mhtmet to be near unity, allowing events to also evade the \mhtmet <
1.25 requirement of the signal region.

To investigate events of this type a study region is defined, populated by
...
\alphat requirement allows events with low \alphat values to be studied.
The region is
