...
\begin{table}[ht!]
  ...
  {\chiz}=240$ \gev and $m_{\sTop} = 250$ \gev, $m_{\chiz}=170$ \gev) of the T2cc
  signal model.} \label{tab:t2cc_cutflow}
  \centering
  \footnotesize
  \begin{tabular}{ lcc }
    \hline
...
\begin{table}[ht!]
  \caption{Cutflow table for the $m_{\sTop} = 250$ \gev, $m_{\chiz}=240$ \gev
  mass point of the \texttt {T2Degen} signal model.}
  ...
  \begin{tabular}{ lcc }
    ...
    Cut Name    & Eff (\%) & N \\
    \hline
    ($m_{\sTop}$, $m_{\chiz}$)& (250, 240) \\
    \hline
    Event Counter & 100.00  \\
    $\nj \geq 2$  & 10.80 \\
    MET Filters & 10.80  \\
    Vertex Noise Filter & 10.80  \\
    HBHE Noise Filter & 10.80  \\
    DeadECAL Filter & 9.13 \\
    $n_{e} = 0$ & 9.13 \\
    $n_{\gamma} = 0$  & 9.13 \\
    $n_{\mu} = 0$ & 9.13 \\
    $\text{EMF}_{max}$ for all jets > 0.1 & 9.13  \\
    Leading jet \Pt > 100 \gev  & 6.01 \\
    Leading jet $\eta$ < 2.5  & 5.70 \\
    Sub-Leading jet \Pt > 100 \gev  & 1.64\\
    $n_{j, fail} = 0$ & 1.61 \\
    $\Delta R(\mu^i_{fail}, jet^j) < 0.5$ & 1.61 \\
    $(\sum_{}^{n_{vertices}}{\Pt}$) / \HT & 1.61  \\
    recHitCut & 1.61  \\
    $n_{SIT} = 0$ & 1.50 \\
    \mindphistar > 0.3  & 1.35 \\
    \mhtmet < 1.25  & 1.24 \\
    \HT > 375 \gev  & 0.55 \\
    \alphat > 0.55  & 0.26 \\
    \mhtmet < 1.25  & 0.26 \\
