...
\begin{table}[!h]
  \caption{
    Ratios of efficiencies ($\epsilon_{\text{MC}}/\epsilon_{\text{data}}$) for
    the dead ECAL filter obtained from data and
    simulation in the \mj control sample with an inclusive selection
    on \nb and \HT ($>200\gev$), and for the two jet multiplicity
    bins, \njlow and \njhigh.  
%    Efficiencies for the dead ECAL filter obtained
%    from data ($\epsilon_{\text{data}}$) and simulation
%    ($\epsilon_{\text{MC}}$) in the \mj control sample with an
%    inclusive selection on \nb and \HT ($>200\gev$), and for the
%    two jet multiplicity bins, \njlow and \njhigh. Also quoted are
%    the ratios of these efficiencies ($\epsilon_{\rm
%      MC}/\epsilon_{\text{data}}$), which is representative of the
